...
const customIconMap: Record<Exclude<CustomIconName, 'bada'>, CustomIconComponent> = {
  github: GitHubIcon,
...
export default function CustomIcon({ name, width = 24, height = 24, ...rest }: CustomIconProps) {
  // bada는 이미지 기반이므로 별도 처리
  if (name === 'bada') {
    return <BadaIcon width={Number(width)} height={Number(height)} className={rest.className} />
  }
